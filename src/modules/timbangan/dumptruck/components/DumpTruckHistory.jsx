...
const DumpTruckHistory = () => {
  ...
  // ✅ Use dateUtils for date range initialization
  ...
  const handleResetFilters = useCallback(() => {
    setSearchQuery("");
    setDateRange(getTodayDateRange()); // ✅ Use dateUtils
    setShifts([]);
    setExcavators([]);
    setWorkUnits([]);
    setLoadingLocations([]);
        ...
        <table className="w-full table-auto">
          ...
          <tbody>
            {paginatedFleets.map((fleet, index) => {
              ...
              return (
                <tr key={fleet.id} className="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700">
