...
export const DEFAULT_BELT_CONVEYOR_CONFIGS = [
  {
    id: 1,
    hauler: "Load In CHF 4",
    loader: "Reclaim Feeder SBR 03",
    loading_point: "Banko Barat - LS DH 4",
    dumping_point: "Banko Barat - DH 4 Jembatan",
    distance: 5000,
    measurement_type: "Beltscale",
    status: "Pemindahan Belt Conveyor",
  },
  {
    id: 2,
    hauler: "Load In CHF 4",
    loader: "Reclaim Feeder SBR 02",
    loading_point: "Banko Barat - LS DH 4",
    dumping_point: "Banko Barat - DH 4 Sumuran",
    distance: 5000,
    measurement_type: "Beltscale",
    status: "Pemindahan Belt Conveyor",
  },
  {
    id: 3,
    hauler: "Load In CHF 5",
    loader: "Reclaim Feeder Breaker",
    loading_point: "Banko Barat - RF 5",
    dumping_point: "Banko Barat - SP 5",
    distance: 8500,
    measurement_type: "Beltscale",
    status: "Pemindahan Belt Conveyor",
  },
  {
    ...
    hauler: "CHF SS 8",
    loader: "Reclaim Feeder Breaker",
    loading_point: "Banko Tengah - LS Sumsel 8",
    dumping_point: "Banko Tengah - PLTU SS 8",
    distance: 1500,
    measurement_type: "Beltscale",
    status: "Pengeluaran Belt Conveyor",
  },
  {
    id: 5,
    hauler: "CHF BPI",
    loader: "Reclaim Feeder 10",
    loading_point: "MTB - RF 10",
    dumping_point: "MTB - PLTU Banjarsari",
    distance: 1000,
    measurement_type: "Beltscale",
    status: "Pengeluaran Belt Conveyor",
  },
  {
    id: 6,
    hauler: "CHF PLTU BA",
    loader: "Crusher PLTU BA T1",
    loading_point: "TAL - SP 1 COALFEEDER",
    dumping_point: "PLTU Bukit Asam",
    distance: 500,
    measurement_type: "Beltscale",
    status: "Pengeluaran Belt Conveyor",
  },
...
const DashboardTab = ({
  ...
  const confirmCoalTypeChange = async () => {
    ...
    try {
      if (settingId) {
        ...
        await onUpdateCoalType({
          id: settingId,
          payload: { coal_type: coalTypeNum },
        });
      } else {
        ...
        if (!config) {
          console.error(
            "[confirmCoalTypeChange] Config tidak ditemukan untuk loader:",
            loader,
          );
          setPendingCoalChange(null);
          setEditingCoalType(null);
  ...
  const checkIsAddLocked = useCallback(
    (hour) => {
      ...
      const filterDate = filters.dateRange?.from
  ...
  const aggregatedData = useMemo(() => {
    return DEFAULT_BELT_CONVEYOR_CONFIGS.map((config) => {
      const records = (data || []).filter(
        (item) => item.loader === config.loader,
      );
      ...
      if (records.length > 0) {
        const dates = [
          ...new Set(records.map((r) => r.date?.split("T")[0]).filter(Boolean)),
        ];
        if (dates.length === 1) {
          ...
          coalTypeItems.find(
            (c) =>
              String(c.label).trim().toLowerCase() ===
                String(beCoalType).trim().toLowerCase() ||
      ...
      return (
        item.loader.toLowerCase().includes(s) ||
  ...
  return (
    <Card className="shadow-sm border-slate-200 dark:border-slate-700 text-slate-900 dark:text-slate-100">
        ...
        <CardTitle className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              className="h-7 px-2.5 text-[11px] font-semibold border-emerald-200 text-emerald-700 hover:bg-emerald-50 dark:border-emerald-800 dark:text-emerald-400 dark:hover:bg-emerald-900/20 gap-1.5"
              title="Refresh daftar coal type dari server"
            >
              <DatabaseZap
                className={`w-3.5 h-3.5 ${mastersLoading ? "animate-pulse" : ""}`}
              />
              {mastersLoading ? "Updating..." : "Refresh Master Data"}
            </Button>
            <Badge
              variant="secondary"
        ...
        <div className="overflow-x-auto scrollbar-thin border border-slate-200 dark:border-slate-700 rounded-lg shadow-sm">
          <table className="w-full text-sm shrink-0">
            <thead className="bg-slate-50 dark:bg-slate-900 border-b border-slate-200 dark:border-slate-700">
              <tr>
                <th className="px-3 py-3 text-left font-semibold sticky left-0 z-10 bg-slate-50 dark:bg-slate-900 border-r border-slate-200 dark:border-slate-700">
                  No
                </th>
                <th className="px-3 py-3 text-left font-semibold sticky left-10 z-10 bg-slate-50 dark:bg-slate-900 border-r border-slate-200 dark:border-slate-700 min-w-[180px]">
                  Loader
                </th>
                ...
                {displayHours.map((hour) => (
                  <th
                    key={hour}
                    className="px-3 py-3 text-center min-w-[70px] border-r border-slate-200 dark:border-slate-700"
                  >
                    {hour.toString().padStart(2, "0")}:00
                  </th>
                ))}
                <th className="px-3 py-3 text-center font-semibold bg-blue-50 dark:bg-blue-900/30 min-w-[100px] border-r border-slate-200 dark:border-slate-700">
                  Total
                </th>
                <th className="px-3 py-3 text-left font-semibold min-w-[150px] border-r border-slate-200 dark:border-slate-700">
                  Loading
                </th>
                <th className="px-3 py-3 text-left font-semibold min-w-[150px] border-r border-slate-200 dark:border-slate-700">
                  Dumping
            ...
            <tbody className="divide-y divide-slate-200 dark:divide-slate-700 bg-white dark:bg-slate-900">
              {aggregatedData.map((row, idx) => (
                ...
                >
                  <td className="px-3 py-3 sticky left-0 z-10 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300">
                    {idx + 1}
                  </td>
                  ...
                  <td className="px-3 py-3 sticky left-[215px] z-10 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 font-mono text-sm">
                    {row.latestBeltscale !== "-"
                      ? Number(row.latestBeltscale).toLocaleString("en-US", {
                  ...
                  <td className="px-2 py-1 sticky left-[315px] z-10 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 text-xs text-center border-t border-b">
                    ...
                    >
                      <span className="truncate mr-2 font-medium">
                        {row.coalTypeId
                          ? coalTypeItems.find(
                              (c) => c.value === String(row.coalTypeId),
                            )?.label || "Batu Bara"
                          : "Pilih..."}
                      </span>
                  ...
                  <td className="px-3 py-3 text-center bg-blue-50 dark:bg-blue-900/30 border-r border-slate-200 dark:border-slate-700">
                    <span className="inline-block px-3 py-1 bg-blue-600 dark:bg-blue-700 text-white rounded text-xs font-bold">
                      {Number(row.totalTonase).toLocaleString("id-ID", {
                        minimumFractionDigits: 0,
                        maximumFractionDigits: 2,
                      })}
                    </span>
                  ...
                  <td className="px-3 py-3 text-center ">
                    <DropdownMenu className="">
                      ...
                      <DropdownMenuContent
                        align="end"
                        className="dark:text-neutral-50 bg-white dark:bg-slate-700 border-none"
                      >
                        {row.records.length > 0 && (
                          <DropdownMenuItem
                            className="cursor-pointer"
                            onClick={() =>
                              onDetail(row.records[row.records.length - 1])
                            }
                          >
      ...
      <Dialog
        open={!!editingCoalType}
        onOpenChange={() => setEditingCoalType(null)}
      >
        <DialogContent className="sm:max-w-md bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800">
          <DialogHeader className="border-b border-slate-100 dark:border-slate-800 pb-3">
            <DialogTitle className="text-slate-900 dark:text-slate-100 text-lg">
              Konfigurasi Batu Bara
            </DialogTitle>
            <p className="text-sm font-medium text-slate-500 dark:text-slate-400 mt-1">
              {editingCoalType?.loader}
            </p>
          </DialogHeader>
          ...
          <DialogFooter className="border-t border-slate-100 dark:border-slate-800 pt-3 dark:text-neutral-50">
            <Button variant="ghost" onClick={() => setEditingCoalType(null)}>
              Batal
            </Button>
            <Button
              onClick={() => {
                setPendingCoalChange({
                  loader: editingCoalType.loader,
                  val: tempCoalTypeValue,
                });
              }}
...
const BeltConveyorManagement = () => {
  ...
  const handleRefreshMaster = useCallback(async () => {
  ...
  const handleAddWithConfig = useCallback(
    (config, hour) => {
      let targetDate = filters.dateRange?.from
        ? new Date(filters.dateRange.from)
        : new Date();
