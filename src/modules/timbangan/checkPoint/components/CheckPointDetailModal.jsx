...
const TimbanganDetailModal = ({
  ...
  useEffect(() => {
    if (item) {
      setEditedData({ net_weight: item.net_weight || "" });
      setMode("view");
      setErrors({});
    }
  }, [item]);
  ...
  const handleCancel = () => {
    setMode("view");
    setEditedData({ net_weight: extractedData.netWeight });
    setErrors({});
  };

  const handleSaveEdit = async () => {
    const error = validateWeight(editedData.net_weight);
    if (error) {
      setErrors({ net_weight: error });
      return;
    }
    ...
    try {
      await onEdit(item, {
        ...item,
        net_weight: parseFloat(editedData.net_weight),
      });
      setMode("view");
    } catch (error) {
            ...
            <div className="space-y-4">
              ...
              <Card className="dark:bg-gray-900 border-none dark:text-gray-200">
                ...
                <CardContent className="space-y-4">
                  ...
                  {mode === "edit" ? (
                    <div>
                      <Label htmlFor="net_weight">Gross Weight (ton) *</Label>
                      <Input
                        id="net_weight"
                        type="text"
                        inputMode="decimal"
                        ...
                        onChange={(e) => {
                          ...
                          if (value === "") {
                            setEditedData({ net_weight: value });
                            setErrors({});
                            return;
                          }
