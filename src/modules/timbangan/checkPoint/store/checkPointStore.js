  ...
  persist(
    (set, get) => ({
      ...
      _executeIndexRebuild: (configs) => {
        ...
        const selectedTimbanganIds = state.selectedFleetIds || [];
        const selectedDigifleetIds = state.digifleetSelectedIds || [];
        const allSelectedIds = [
          ...selectedTimbanganIds,
          ...selectedDigifleetIds,
        ];
        ...
        if (allSelectedIds.length > 0) {
          filteredConfigs = filteredConfigs.filter((cfg) =>
            allSelectedIds.includes(cfg.id)
          );
        } else {
          console.warn("⚠️ No selected fleets, fallback to ALL ACTIVE fleets");
        }
        ...
        filteredConfigs.forEach((cfg) => {
          ...
          const isDigiFleet = !!cfg.id_digifleet;
          ...
          units.forEach((unit) => {
            ...
            idx[key] = {
              ...
              id_digifleet: cfg.id_digifleet || null,
              isDigiFleet,
      ...
      setDumptruckIndexFromConfigs: (configs) => {
        ...
        const state = get();
        if (state.selectedFleetIds.length > 0) {
          get()._executeIndexRebuild(configs);
        } else {
      ...
      setDigifleetSelected: (ids) => {
        const safeIds = Array.isArray(ids) ? ids : [];
        set({ digifleetSelectedIds: safeIds });
        get()._scheduleIndexRebuild();
      },

      addDigifleetSelected: (fleetId) => {
        set((state) => {
          const newIds = [...new Set([...state.digifleetSelectedIds, fleetId])];
          return { digifleetSelectedIds: newIds };
        });
        get()._scheduleIndexRebuild();
      },

      removeDigifleetSelected: (fleetId) => {
        set((state) => {
          const newIds = state.digifleetSelectedIds.filter(
            (id) => id !== fleetId
          );
          return { digifleetSelectedIds: newIds };
        });
      ...
      toggleDigifleetSelected: (fleetId) => {
        set((state) => {
          const isSelected = state.digifleetSelectedIds.includes(fleetId);
          const newIds = isSelected
            ? state.digifleetSelectedIds.filter((id) => id !== fleetId)
            : [...state.digifleetSelectedIds, fleetId];
      ...
      clearDigifleetSelected: () => {
        set({ digifleetSelectedIds: [] });
        get()._scheduleIndexRebuild();
      },

      getDigifleetSelected: () => {
        const state = get();
        return state.fleetConfigs.filter(
          (f) => state.digifleetSelectedIds.includes(f.id) && f.id_digifleet
        );
      },
      ...
      findByHullNo: (hullNo, includeHidden = false) => {
        ...
        const result = state.dtIndex[key] || null;
      ...
      addFleetConfig: (config) =>
        set((state) => {
          ...
          if (!newConfig.id_digifleet && newConfig.status === "ACTIVE") {
      ...
      updateFleetConfig: (configId, updates) =>
        set((state) => {
          const oldConfig = state.fleetConfigs.find((c) => c.id === configId);
          ...
          if (oldConfig && !oldConfig.id_digifleet) {
      ...
      deleteFleetConfig: (configId) =>
        set((state) => {
          ...
          const newDigifleetSelectedIds = state.digifleetSelectedIds.filter(
            (id) => id !== configId
          );
          ...
          return {
            ...
            digifleetSelectedIds: newDigifleetSelectedIds,
      ...
      resetSetup: () =>
        set({
          ...
          digifleetSelectedIds: get().digifleetSelectedIds,
    ...
    }),
    {
      ...
      partialize: (state) => ({
        ...
        digifleetSelectedIds: state.digifleetSelectedIds,
