...
const [rfidMode] = useState(true); 
  ...
  const [rfidTapDetected, setRfidTapDetected] = useState(false);
  const [lastRfidUid, setLastRfidUid] = useState("");
  const [autoSubmitting, setAutoSubmitting] = useState(false);

  const rfidBufferRef = useRef("");
  const rfidLastInputRef = useRef(0);
  ...
  useEffect(() => {
   ...
   useEffect(() => {
    if (!rfidMode || !rfidConnected) return;

    sendWeightStable(isWeightStable && insertedWeight !== null);

  }, [isWeightStable, insertedWeight, rfidMode, rfidConnected, sendWeightStable]);
  ...
  const handleRFIDKeyInput = (e) => {
    if (!rfidMode) return;

    const now = Date.now();

    if (now - rfidLastInputRef.current > 100) {
      rfidBufferRef.current = "";
    }

    rfidLastInputRef.current = now;

    if (e.key === "Enter") {
      const uid = rfidBufferRef.current.trim();
      rfidBufferRef.current = "";
      ...
      const match = Object.values(dtIndex).find((dt) => dt.rfid_uid === uid);

      if (!match) {
        console.warn("❌ RFID not recognized:", uid);
        setRfidTapDetected(false);
        return;
      }

      setRfidTapDetected(true);
      ...
      if (rfidWaitingSubmit && insertedWeight !== null && isValid) {
  ...
  useEffect(() => {
    ...
    window.addEventListener("keydown", handleRFIDKeyInput);
    return () => {
      window.removeEventListener("keydown", handleRFIDKeyInput);
    };
  }, [rfidMode, dtIndex, rfidWaitingSubmit, insertedWeight, isValid, formData]);
 ...
 const handleAutoSubmitAfterRFID = async () => {
    ...
    try {
      ...
      if (result.success && onSubmit) {
        ...
        // Reset form untuk ritase berikutnya
        setTimeout(() => {
          setInsertedWeight(null);
          setInsertedTime(null);
          setDisplayWeight("");
  ...
  const selectedDT =
    formData.hull_no &&
    dtIndex[formData.hull_no.replace(/\s+/g, "").toUpperCase()];
  ...
  function handleRfidScanFromBackend(scanData) {

    if (!scanData || !scanData.epc) {
      console.warn("⚠️ Invalid scan data");
      return;
    }

    setLastRfidEpc(scanData.epc);

    // Cari di dtIndex berdasarkan RFID UID/EPC
    const match = Object.values(dtIndex).find(
      (dt) => dt.rfid_uid === scanData.epc || dt.rfid_epc === scanData.epc
    );

    if (!match) {
      console.warn("❌ RFID not found in dtIndex:", scanData.epc);
      setRfidMatchStatus({
        success: false,
    ...
    // Auto-fill form dengan data dari dtIndex
    updateField("hull_no", match.hull_no);
    updateField("dumptruckId", match.dumptruckId);
    updateField("operator", match.operator_name);
    updateField("operatorId", match.operator_id);
    ...
    // 🚀 Auto-submit jika weight sudah locked dan form valid
  ...
  // ===================================================================
  // 🚀 Auto-submit setelah RFID scan
  // ===================================================================

 

  // ===================================================================
  // 🎨 RFID Status Indicator UI
  // ===================================================================
  const renderRfidStatus = () => {
    ...
    return (
      <Card className={`border-2 ${canScan ? 'border-green-300 bg-green-50' : 'border-gray-300 bg-gray-50'}`}>
        <CardContent className="py-3">
          <div className="flex items-center justify-between gap-2">
            <div className="flex items-center gap-2">
              <div className={`w-3 h-3 rounded-full ${canScan ? 'bg-green-600 animate-pulse' : 'bg-gray-400'}`} />
              <span className="text-sm font-semibold">
                RFID: {canScan ? '✅ Ready to Scan' : '⏸️ Waiting for Weight Lock'}
              </span>
            </div>
          ...
          {rfidMatchStatus && (
            <div className={`mt-2 pt-2 border-t ${
              rfidMatchStatus.success ? 'border-green-200' : 'border-red-200'
            }`}>
              <div className={`text-xs ${
                rfidMatchStatus.success ? 'text-green-700' : 'text-red-700'
              }`}>
                {rfidMatchStatus.success ? '✅' : '❌'} {rfidMatchStatus.message}
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-3">
      ...
      {rfidMode && (rfidWaitingSubmit || rfidTapDetected || autoSubmitting) && (
        <Card className="border-purple-200 bg-linear-to-r from-purple-50 to-purple-100 shadow-sm">
          <CardContent className="py-3">
            <div className="flex items-center justify-between gap-4">
              <div className="flex items-center gap-3 flex-1">
                <div className="bg-white p-2 rounded-lg shadow-sm">
                  {autoSubmitting ? (
                    ...
                    <CreditCard className="w-5 h-5 text-purple-600 animate-pulse" />
                  )}
                </div>
                <div className="flex-1">
                  ...
                  <div className="text-xs text-purple-700 mt-0.5">
                    {autoSubmitting
                      ...
                      : rfidTapDetected
                      ? `${formData.hull_no} - ${formData.operator}`
                      : "Weight locked. Tap kartu RFID untuk submit otomatis"}
                  </div>
                </div>
              </div>
              {lastRfidUid && (
                <Badge className="bg-purple-600 text-xs font-mono">
                  {lastRfidUid.slice(0, 8)}...
                </Badge>
              )}
            </div>
          </CardContent>
          ...
          <CardContent className="py-3">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-sm font-medium text-green-800">
                Fleet: {formSummary.fleetInfo.name}
              </span>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
                ...
                <div className="space-y-1">
                  ...
                  <Badge variant="outline" className="text-xs">
                    {formSummary.fleetInfo.shift}
                  </Badge>
                  <div className="text-gray-500 mt-2">Tanggal</div>
                  <div className="font-medium">
                    {new Date(formSummary.fleetInfo.date).toLocaleDateString(
                      "id-ID",
      ...
      {formData.hull_no && selectedDT?.isHidden && (
        <Alert variant="warning">
          <AlertCircle className="w-4 h-4" />
          <AlertDescription>
            <p className="font-medium mb-1">⚠️ Dump truck sudah ditimbang</p>
            <p className="text-sm">
              Nomor lambung <strong>{formData.hull_no}</strong> sudah disubmit
      ...
      {formData.hull_no && !selectedDT && (
        <Alert variant="destructive">
          <AlertCircle className="w-4 h-4" />
          <AlertDescription>
            <p className="font-medium mb-1">❌ Nomor lambung tidak ditemukan</p>
            <p className="text-sm">
      ...
      <div>
        <div className="pt-2">
          ...
          <div className="mt-2">
            ...
            {rfidMode && rfidWaitingSubmit && !formData.hull_no && (
