...
const buildFilters = (options = {}) => {
  ...
  switch (role) {
    ...
    case "ccr": {
      ...
      if (!satker) {
        logger.warn("⚠️ CCR: satker tidak ditemukan", { userId: user.id });
        return {
          needsFeedback: true,
          message:
            "Data tidak dapat difilter karena satker tidak ditemukan. Silakan hubungi admin.",
        };
      }
    ...
    case "evaluator":
    case "pic": {
      const userSatker = user?.work_unit?.satker;
      if (userSatker) {
        filters.pic_work_unit = {
          satker: { $eq: userSatker },
        };
