...
export const KinerjaRakorChart = ({ data = [], loading = false, title = "Statistik Rakor" }) => {
  if (loading) {
    return (
  ...
  // Cari max % untuk menyeimbangkan YAxis kanan
    ...
    return (
      <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-4 h-[400px] flex flex-col">
        <h3 className="text-lg font-bold text-slate-800 dark:text-slate-100 flex items-center gap-2 mb-6">
          <div className="w-8 h-8 rounded-full bg-orange-100 flex items-center justify-center text-orange-600 text-xs">0%</div>
          {title}
        </h3>
        <div className="flex-1 flex items-center justify-center text-slate-400">
          Tidak ada data Rakor
        </div>
      </div>
    );
  }
    ...
    <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-4 flex flex-col h-[450px]">
      <div className="flex justify-between items-start mb-2">
         <div className="flex items-center gap-3">
           <div className="w-10 h-10 rounded-full bg-orange-400 flex items-center justify-center text-white font-bold text-sm shadow-md">
      ...
      <div className="flex-1 mt-2 w-full h-full min-h-0">
          ...
          <tbody>
             <tr className="bg-slate-50 dark:bg-slate-800/50">
               <td className="p-1 border-r border-slate-100 dark:border-slate-700 text-left font-medium flex items-center gap-1">
                  <div className="w-2 h-2 bg-amber-400 rounded-sm"></div> Realisasi
               </td>
               {data.map((col, idx) => (
             ...
             <tr>
               <td className="p-1 border-r border-slate-100 dark:border-slate-700 text-left font-medium flex items-center gap-1">
                 <div className="w-2 h-2 bg-orange-600 rounded-full"></div> Target
               </td>
               {data.map((col, idx) => (
