...
const ScaleConnectionStatusRFID = ({
  isSupported,
  isAutoConnecting,
  connectionTimeout,
  wsConnected,
  ...
  const renderScaleButton = () => {
    if (isConnecting || isAutoConnecting) {
      return (
        <Button size="sm" disabled className=" text-white">
  ...
  const colorClasses = {
    blue: "border-blue-200 dark:border-blue-800 text-blue-900 dark:text-blue-200",
    orange:
  ...
  return (
    <div
      className={`flex items-center gap-2 px-3 py-2 rounded-md ${colorClasses[statusColor]}`}
    >
      {/* Scale Section */}
...
export default ScaleConnectionStatusRFID;
