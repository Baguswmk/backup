...
const SuperAppsHub = () => {
  ...
  // Get apps grouped by category (bukan by type)
  const timbanganApps = getUserApps().filter(app => 
    app.key.startsWith('timbangan')
  );
  const businessApps = getUserApps().filter(app => 
    !app.key.startsWith('timbangan')
  );

  // Check if app is accessible (has path or url)
  const isAppAccessible = (app) => {
    // Check if has url or path
    const hasUrl = app.url && app.url.trim() !== '';
    const hasPath = app.path && app.path.trim() !== '';
    return hasUrl || hasPath;
  };

  // Handle app click - UPDATED: support both url and path
  const handleAppClick = (app) => {
    // Don't do anything if app is not accessible
    if (!isAppAccessible(app)) {
      return;
    }

    // Prioritas: gunakan url jika ada, kalau tidak gunakan path
    ...
    return (
      ...
      >
        ...
        <CardContent className="pt-0">
          <p className={`text-sm text-center mb-4 min-h-10 flex items-center justify-center ${
          ...
          <div className="flex justify-between items-center py-4">
            <div className="flex items-center space-x-4">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h1 className="text-xl font-bold text-gray-900 dark:text-gray-100">Application Hub</h1>
                  <p className="text-xs text-gray-500 dark:text-gray-400">Select an application to continue</p>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center space-x-4">
              ...
              <LogoutConfirmationDialog 
                ...
                buttonProps={{
                  className: "text-red-600 dark:text-red-400 hover:text-red-700 dark:hover:text-red-300 hover:bg-red-50 dark:hover:bg-red-900/30 border-red-200 dark:border-red-800 transition-colors"
                }}
              />
            </div>
          </div>
          ...
          <Card className="text-center py-16 border border-gray-200 dark:border-gray-700 dark:bg-gray-800">
            <CardContent>
              ...
              <p className="text-gray-600 dark:text-gray-400 mb-8 max-w-md mx-auto">
                You don't have access to any applications yet. Please contact your administrator to request access to the applications you need.
              </p>
              <div className="bg-gray-50 dark:bg-gray-700 rounded-xl p-6 inline-block">
                <p className="text-sm text-gray-600 dark:text-gray-300">
                  Your current role: <span className="font-semibold text-gray-900 dark:text-gray-100">{userRole}</span>
                </p>
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                  Contact your system administrator to get access to applications
                </p>
              </div>
            </CardContent>
          </Card>
