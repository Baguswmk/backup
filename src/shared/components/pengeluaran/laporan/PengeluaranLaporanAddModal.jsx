...
const ManualForm = ({
  ...
  const validate = () => {
    ...
    if (
      new Date(header.end_loading_time) <= new Date(header.start_loading_time)
    )
    ...
    for (let i = 0; i < wagons.length; i++) {
      ...
      if (
        wagons[i].load_weight === "" ||
        isNaN(Number(wagons[i].load_weight)) ||
        Number(wagons[i].load_weight) <= 0
      )
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="col-span-2 sm:col-span-1">
            <label className={labelCls}>
              ID Rangkaian (BBR) <span className="text-red-500">*</span>
            </label>
            <input
              className={inputCls}
          ...
          <div className="col-span-2">
            <label className={labelCls}>
              Loading Location (Asal) <span className="text-red-500">*</span>
            </label>
            <SearchableSelect
              id="manual-origin"
          ...
          <div className="col-span-2">
            ...
            <SearchableSelect
              ...
              items={(dumpingLocations.length > 0 ? dumpingLocations : []).map(
                (loc) => ({ value: loc.name, label: loc.name }),
              )}
          ...
          <div>
            <label className={labelCls}>
              Mulai Muat <span className="text-red-500">*</span>
            </label>
            <input
              type="datetime-local"
              className={inputCls}
              ...
              onChange={(e) =>
                setHeaderField("start_loading_time", e.target.value)
              }
              disabled={isSubmitting}
            />
          </div>
          ...
          <div>
            <label className={labelCls}>
              Selesai Muat <span className="text-red-500">*</span>
            </label>
            <input
              type="datetime-local"
              className={inputCls}
              ...
              onChange={(e) =>
                setHeaderField("end_loading_time", e.target.value)
              }
              disabled={isSubmitting}
            />
          </div>
            ...
            <thead className="bg-slate-50/80 dark:bg-slate-800/40 text-slate-400 dark:text-slate-500 text-[10px] uppercase tracking-widest font-bold border-b border-slate-200 dark:border-slate-800">
              <tr>
                ...
                <th className="px-4 py-2.5 min-w-[150px]">
                  Nomor Gerbong <span className="text-red-500/80">*</span>
                </th>
                <th className="px-4 py-2.5 min-w-[120px]">Jenis BB</th>
                <th className="px-4 py-2.5 min-w-[110px] text-right">
                  Tonase (ton) <span className="text-red-500/80">*</span>
                </th>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {wagons.length === 0 ? (
                <tr>
                  <td
                    colSpan={5}
                    className="px-4 py-10 text-center text-xs text-slate-400 dark:text-slate-500 italic"
                  >
                    Belum ada gerbong. Klik &quot;Tambah Gerbong&quot; untuk
                    memulai.
                  </td>
                </tr>
              ) : (
                  ...
                  >
                    ...
                    <td className="px-3 py-1.5">
                      <input
                        ...
                        onChange={(e) =>
                          updateWagon(wagon._id, "coal_type", e.target.value)
                        }
                        disabled={isSubmitting}
                      />
                    </td>
                    <td className="px-3 py-1.5">
                      <input
                        ...
                        onChange={(e) =>
                          updateWagon(wagon._id, "load_weight", e.target.value)
                        }
                        disabled={isSubmitting}
                        min={0}
                        step={0.001}
              ...
              <tfoot className="bg-gray-50 dark:bg-slate-800/30 text-xs font-medium text-gray-600 dark:text-gray-300">
                <tr>
                  ...
                  <td className="px-3 py-2 text-right font-mono font-bold text-green-700 dark:text-green-400">
                    {wagons
                      .reduce((sum, w) => sum + (Number(w.load_weight) || 0), 0)
                      .toFixed(3)}{" "}
                    ton
                  </td>
                  <td />
...
export const PengeluaranLaporanAddModal = ({
  ...
  const [loadingLocation2, setLoadingLocation2] = useState("");
  const [selectedForLoc2, setSelectedForLoc2] = useState([]);
  ...
  const { locations, isLoading: isLoadingLocations } =
    useMasterData("locations");

  const loadingLocations = (locations || []).filter(
    (loc) => loc.type === "LOADING",
  );
  const dumpingLocations = (locations || []).filter(
    (loc) => loc.type === "DUMPING",
  );
  ...
  const handleSubmitUpload = async () => {
    ...
    if (showLocation2 && selectedForLoc2.length > 0 && !loadingLocation2) {
      setErrorMsg("Harap pilih Lokasi Kedua (jika digunakan).");
      return;
    }

    const finalPayload = parsedData.map((item, index) => ({
      ...item,
      loading_location:
        showLocation2 && selectedForLoc2.includes(index)
          ? loadingLocation2
          : loadingLocation,
    }));
  ...
  const handleOverrideSubmit = async () => {
    ...
    if (onSubmitOverride && duplicateError?.payload) {
      setUploadError("");
      const success = await onSubmitOverride(
  ...
  if (duplicateError) {
    ...
    const canSubmit =
      overrideNote.trim() && evidenceId && !isUploadingBA && !isSubmitting;
          ...
          {!isFormPhase && (
            <div className="flex flex-col gap-5 py-2">
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400 bg-orange-50 dark:bg-orange-900/20 border border-orange-200 dark:border-orange-800 rounded-md px-3 py-2">
                ⚠️ Proses ini akan <strong>menghapus</strong> seluruh data yang
                ada dan menggantinya dengan data baru. Anda wajib menyertakan
                dokumen Berita Acara sebagai bukti perubahan.
              </p>
              ...
              <div>
                ...
                {!evidenceFile ? (
                  ...
                  >
                    ...
                    />
                    <svg
                      className="w-8 h-8 text-orange-400 mb-2"
                      fill="none"
                      viewBox="0 0 24 24"
                      stroke="currentColor"
                    >
                  ...
                  <div className="flex items-center gap-3 border border-orange-200 dark:border-orange-800 bg-orange-50 dark:bg-orange-900/20 rounded-lg px-4 py-3">
                    <svg
                      className="w-8 h-8 text-orange-500 shrink-0"
                      fill="none"
                      viewBox="0 0 24 24"
                      stroke="currentColor"
                    >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {isUploadingBA ? (
                        <p className="text-xs text-orange-500 animate-pulse mt-0.5">
                          Mengupload dokumen...
                        </p>
                      ) : evidenceId ? (
                        <p className="text-xs text-green-600 dark:text-green-400 mt-0.5 font-medium">
                          ✓ Berhasil diupload (ID: {evidenceId})
                        </p>
                      ) : null}
                    </div>
                    <button
    ...
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto bg-white dark:bg-slate-900 dark:text-neutral-50 flex flex-col">
        ...
        >
          ...
          <TabsContent
            value="excel"
            className="flex flex-col flex-1 h-full mt-0 border-none p-0 outline-none"
          >
            {errorMsg && (
            ...
            <div className="mb-4 flex flex-col gap-3">
              <div>
                <div className="flex items-center justify-between mb-1">
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
                    Loading Location Utama{" "}
                    <span className="text-red-500">*</span>
                  </label>
                  {!showLocation2 && (
                    <button
                      type="button"
                      onClick={() => setShowLocation2(true)}
                      className="text-xs text-blue-600 dark:text-blue-400 hover:underline flex items-center gap-1 font-medium"
                    >
                      <PlusIcon /> Tambah Lokasi Lain
                    </button>
                  )}
                </div>
                <SearchableSelect
                  ...
                  placeholder="-- Pilih Loading Location Utama --"
                  allowClear
                />
              </div>
              ...
              {showLocation2 && (
                <div className="p-3 bg-blue-50/50 dark:bg-blue-900/10 border border-blue-100 dark:border-blue-900/30 rounded-lg">
                  ...
                  <SearchableSelect
                    id="loading-location-2"
                    items={DEFAULT_LOADING_LOCATIONS}
                    value={loadingLocation2}
                    onChange={setLoadingLocation2}
                    placeholder="-- Pilih Lokasi Kedua --"
                    allowClear
                  />
                  <p className="text-[10px] text-gray-500 mt-1.5 flex items-center gap-1.5">
                    ...
                    Centang gerbong pada tabel di bawah untuk memindahkannya ke
                    lokasi ini.
                  </p>
                </div>
              )}
            ...
            {!selectedFile ? (
              ...
              <div className="border border-gray-200 dark:border-slate-700 rounded-lg p-4 bg-gray-50 dark:bg-slate-800/30 flex flex-col gap-4">
                ...
                {!importing && parsedData.length > 0 && (
                  <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-lg overflow-hidden flex flex-col">
                    <div className="bg-slate-100 dark:bg-slate-800 px-3 py-2 text-xs font-semibold text-gray-600 dark:text-gray-300 border-b border-gray-200 dark:border-slate-700 flex justify-between items-center">
                      <span>Preview Data ({parsedData.length} baris)</span>
                      {showLocation2 && (
                        <span className="text-[10px] font-medium bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300 px-2 py-0.5 rounded-full">
                          {selectedForLoc2.length} terpilih untuk Lokasi 2
                        </span>
                      )}
                    </div>
                      ...
                      <table className="w-full text-left border-collapse text-xs whitespace-nowrap">
                        ...
                        <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                          {parsedData.map((row, i) => (
                            <tr
                              key={i}
                              className={`hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors ${showLocation2 ? "cursor-pointer" : ""} ${showLocation2 && selectedForLoc2.includes(i) ? "bg-blue-50/30 dark:bg-blue-900/10" : ""}`}
                              onClick={() => {
                                if (showLocation2) {
                                  setSelectedForLoc2((prev) =>
                                    prev.includes(i)
                            ...
                            >
                              {showLocation2 && (
                                <td className="px-3 py-2 text-center border-l border-gray-100 dark:border-slate-800 bg-blue-50/10 dark:bg-blue-900/5">
                                  <input
                                    type="checkbox"
                                    className="rounded text-blue-600 focus:ring-blue-500 h-4 w-4 pointer-events-none"
                                    checked={selectedForLoc2.includes(i)}
                                    readOnly
                                  />
                                </td>
                              )}
          ...
          >
            <ManualForm
              loadingLocations={DEFAULT_LOADING_LOCATIONS.map((item) => ({
                name: item.value,
              }))}
